...
class FilterAlgTest extends AnyFunSuite with Matchers {
  ...
  test("ignore update via config updates.pin using suffix") {
    ...
    localFilter(update, config) shouldBe Right(update.copy(newerVersions = Nel.of("7.3.0.jre8")))
  }
  ...
  test("localFilter: includeScala = false") {
    val update = Single(
      "org.scala-lang" % ArtifactId("scala-compiler", "scala-compiler_2.12") % "2.12.10",
      Nel.of("2.12.11")
    )
    localFilter(update, config) shouldBe Left(IgnoredByConfig(update))
  }

  test("localFilter: includeScala = true") {
    val update = Single(
      "org.scala-lang" % ArtifactId("scala-compiler", "scala-compiler_2.12") % "2.12.10",
      Nel.of("2.12.11")
    )
    val config = RepoConfig.empty.copy(updates = UpdatesConfig(includeScala = Some(true)))
    localFilter(update, config) shouldBe Right(update)
  }
